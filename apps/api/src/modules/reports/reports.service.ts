...
export class ReportsService {
  private refundColumnPromise: Promise<boolean> | null = null;
  ...
  async revenue(
    ...
    const hasRefundColumn = await this.hasRefundColumn();
    ...
    const [periodRows, methodRows, packageRows, totalRow, refundRow] = await Promise.all([
      ...
      hasRefundColumn
        ? this.prisma.$queryRaw<{ refund_total: Prisma.Decimal | null }[]>(Prisma.sql`
            SELECT SUM(p."refundedAmount") AS refund_total
            FROM payments p
            WHERE p.studio_id = ${tenant.studioId}::uuid
              AND p.paid_at >= ${range.from} AND p.paid_at < ${range.to}
              ${branchSql}`)
    ...
    return {
      ...
      byMethod,
      byPackage,
      refundTotal: refundRow ? decimalOrZero(refundRow[0]?.refund_total ?? null).toFixed(2) : null,
    };
  }
  ...
  /** Payment.refundedAmount may not exist yet (schema is defensive); check once and cache. */
  private hasRefundColumn(): Promise<boolean> {
    if (!this.refundColumnPromise) {
      this.refundColumnPromise = this.prisma
        .$queryRaw<{ exists: boolean }[]>(
          Prisma.sql`SELECT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'payments' AND column_name = 'refundedAmount'
          ) AS exists`,
        )
        .then((rows) => rows[0]?.exists ?? false);
    }
    return this.refundColumnPromise;
  }
}
