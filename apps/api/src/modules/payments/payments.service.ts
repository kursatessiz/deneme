...
export class PaymentsService {
  ...
  async refundPayment(tenant: TenantContext, actorUserId: string, paymentId: string, dto: RefundPaymentInput) {
    ...
    if (requested <= 0 || requested > remaining + 1e-6) {
    ...
    if (payment.providerReference) {
      ...
      const refundResult = await this.providers.get(providerName).refund({
        studioId,
    ...
    const newRefunded = Number(payment.refundedAmount) + requested;
    const fullyRefunded = newRefunded >= Number(payment.amount) - 1e-6;

    // Conditional update on the snapshot read above: a concurrent refund of
    // the same payment cannot both succeed and push refundedAmount past amount.
    const updated = await this.prisma.payment.updateMany({
      where: { id: payment.id, studioId, refundedAmount: payment.refundedAmount },
      data: {
        refundedAmount: newRefunded,
        paymentStatus: fullyRefunded ? PaymentStatus.REFUNDED : payment.paymentStatus,
      },
    });
    if (updated.count === 0) {
    ...
    await this.prisma.auditLog.create({
      data: {
        ...
        entityType: 'Payment',
        entityId: payment.id,
        metadata: { amount: requested, reason: dto.reason },
      },
    });
