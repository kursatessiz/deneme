 ...
 * (GET /calendar/:token.ics). Keyed by client IP only, so one client
 * cannot scan many tokens by varying the URL. Calendar apps poll a feed a
 * few times an hour, far below the limit. No-ops (fails open) when Redis is
...
export class CalendarFeedRateLimitGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const key = `calendar-feed-rl:${req.ip ?? 'unknown'}`;
