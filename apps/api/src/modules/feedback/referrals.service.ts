...
export class ReferralsService {
   ...
   */
  /**
   * Scheduler sweep (JobsService heartbeat): re-evaluates open referrals so
   * qualification and rewards happen without anyone opening a list.
   */
  async recomputeOpen(limit = 500): Promise<{ evaluated: number }> {
    const open = await this.prisma.referral.findMany({
      where: { status: { in: [PrismaReferralStatus.PENDING, PrismaReferralStatus.QUALIFIED] } },
      select: { id: true, studioId: true },
      orderBy: { createdAt: 'asc' },
      take: limit,
    });
    for (const r of open) {
      await this.recomputeOne(r.studioId, r.id);
    }
    return { evaluated: open.length };
  }
