...
describe('Payroll (e2e)', () => {
  ...
  describe('approve, pay, and immutability', () => {
    ...
    it('a studio-wide run cannot overlap approved branch runs (no double pay)', async () => {
      const studioWide = await as(ownerToken)
        .post(`/payroll/studio/${ZEN}/runs`)
        .send({ periodStart: FEB_START.toISOString(), periodEnd: FEB_END.toISOString() });
      expect(studioWide.status).toBe(409);

      // A draft created before the branch approvals cannot be approved either.
      const drafts = await as(ownerToken).get(`/payroll/studio/${ZEN}/runs?status=DRAFT`);
      const wideDraft = (drafts.body.items ?? drafts.body).find((r: any) => r.branchId === null);
      if (wideDraft) {
        const approve = await as(ownerToken).post(`/payroll/studio/${ZEN}/runs/${wideDraft.id}/approve`);
