...
describe('Check-in kiosk and QR (e2e)', () => {
  ...
  const cleanupBookings = async () => {
    // QR/kiosk check-ins run gamification (W16); drop badges these bookings awarded.
    const bookingIds = (await prisma.booking.findMany({ where: { scheduleId: { in: scheduleIds } }, select: { id: true } })).map((b) => b.id);
    await prisma.memberBadge.deleteMany({ where: { sourceRef: { in: bookingIds } } });
