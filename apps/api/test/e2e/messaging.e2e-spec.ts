...
describe('Messaging (e2e)', () => {
  ...
  describe('commercial consent self-service', () => {
    ...
    it('the consent list carries phone numbers, so it needs members.contact.view', async () => {
      const ownerList = await as(ownerToken, ZEN).get(`/studios/${ZEN}/notification-consents`);
      expect(ownerList.status).toBe(200);
      expect(Array.isArray(ownerList.body.items)).toBe(true);

      // Trainers can view members but not their contact details.
      const trainer = await as(trainerToken, ZEN).get(`/studios/${ZEN}/notification-consents`);
    ...
    it('exports a spreadsheet-safe CSV for staff with contact access only', async () => {
      const res = await as(ownerToken, ZEN).get(`/studios/${ZEN}/notification-consents/export`);
      expect(res.status).toBe(200);
      ...
      const trainer = await as(trainerToken, ZEN).get(`/studios/${ZEN}/notification-consents/export`);
      expect(trainer.status).toBe(403);
    });
  });
});
