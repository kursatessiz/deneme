...
akışında **yeniden tarama bir hata değildir** (aşağıya bakınız). Başarılı her
QR/kiosk check-in'i, personel check-in'iyle aynı şekilde oyunlaştırma
değerlendirmesini (W16, `GamificationService.onAttendance`) en iyi çaba ile
tetikler; bu değerlendirme hata verse bile check-in başarılı sayılır.
...
- Aynı rezervasyon için eşzamanlı iki tarama gelirse, durumu değiştiremeyen
  istek rezervasyonu yeniden okur ve ATTENDED ise başarılı döner.
