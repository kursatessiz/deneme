...
- Günlük otomatik hesaplama: 15 dakikalık zamanlayıcı döngüsü
  (`apps/api/src/modules/jobs/jobs.service.ts`) her çalıştığında
  `ChurnService.recomputeStale` çağrılır; yalnızca son hesaplaması 20 saatten
  eski olan aktif stüdyolar yeniden puanlanır, böylece her stüdyo günde
  yaklaşık bir kez güncellenir.
- Kiracı içi elle tetikleme: `POST /churn/studio/:studioId/recompute`
  (`members.manage` izni, stüdyo başına 10 dakikada bir hız sınırlı;
  `audit_logs` üzerinden kontrol edilir).
- Platform geneli zorunlu yeniden hesaplama: `POST /admin/churn/recompute-all`
  (yalnızca süper admin,
  `apps/api/src/modules/churn/churn-admin.controller.ts`).
