...
- **Önemli (Google politikası):** teşvikli (incentivized) yorumlar -- indirim, hediye, ekstra seans hakkı vb.
...
- **Zamanlama:** 15 dakikalık zamanlayıcı döngüsü (`apps/api/src/modules/jobs/jobs.service.ts`) her
  çalıştığında `RatingPromptService.promptRecentAttendees` çağrılır. Süper admin
  `POST /admin/feedback/rating-prompts/run` ile elle de tetikleyebilir (`now` alanı yalnızca `NODE_ENV=test`
  iken dikkate alınır).
...
- **Yeniden değerlendirme ne zaman çalışır**: zamanlayıcı döngüsü her çalıştığında açık (PENDING/QUALIFIED)
  tavsiyeleri `ReferralsService.recomputeOpen` ile yeniden değerlendirir (tek çalıştırmada en fazla 500).
  Liste uç noktaları da aynı kontrolü çağırır. Ödül, referans veren üyenin aktif paketine göreli artırma
  (`increment`) ile eklenir; eşzamanlı hak düşümü kaybolmaz.
