...
- 1.4 `ServiceType`, `Resource`, çakışma kontrolü kaynak bazlı; bekleme listesi; iptal politikası motoru; eğitmen yerine geçme (W1 ile kısmen yapıldı: `catalog` modülü — `ServiceType`, `ResourceType`, `Resource`, `CancellationPolicy` için liste/oluştur/güncelle/pasifleştir uç noktaları, eğitmen niteliği ekleme/kaldırma, çapraz kiracı id doğrulaması, stüdyo başına tek varsayılan iptal politikası kısıtı; `POST /schedules/:scheduleId/cancel-session` ile tüm seansı iptal etme — onaylı rezervasyonları işletme iptali olarak tam iade ile kapatır, kaynak atamalarını pasifleştirir, bekleme listesini iptal eder, `AuditLog` yazar ve üyelere `BOOKING_CHANGE` bildirimi gönderir; kalan: bekleme listesi otomatik doldurma ve eğitmen yerine geçme uç noktaları)
